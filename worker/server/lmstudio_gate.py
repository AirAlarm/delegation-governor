...
This is dg-worker's own copy, not a patch applied on top of someone else's
plugin -- there is no separate upstream to reapply against or drift from.
(The un-forked cc-delegate install, if still present, is patched separately;
see src/dg/ccdelegate/ and docs/STATION_DELEGATION.md in delegation-governor's
own repo.)
"""
