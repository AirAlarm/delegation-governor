...
from dg import distribution, lanes, store
...
class DistributionTest(DGTest):
    ...
    def tearDown(self):
        ...
        sys.modules.pop("dg.routelog", None)  # the run() test injects a fake
        super().tearDown()
...
class TestRun(DistributionTest):
    def test_run_json_prints_a_valid_report(self):
        ...
        # routelog is written by the parallel task; stand in for it
        fake = types.ModuleType("dg.routelog")
        fake.read_routes = lambda: routes
        sys.modules["dg.routelog"] = fake
